<template>
	<div class="content-header" id="booth-stamps">
		<div class="container-fluid">
			<div class="container-fluid py-2">
				<div class="row">
					<div class="col-12 pl-0">
						<button class="btn btn-blue btn-export border" @click="exportStamps">
							Export Stamps
						</button>
					</div>
				</div>
...
export default {
	...
	data() {
		return {
			...
			fields: [
                ...
                { key: 'created_at', label: 'Stamped Date', tdClass: "align-middle", sortable: true },
			],
			...
			perPage: 10,
			stampCriteria: '',
            stampFilterTimeout: null,
			isBusy: true,
		}
	},
	computed: {
		...mapGetters({
            stamps: 'stamps/getStamps',
			user: 'auth/user',
        }),
	},
	methods: {
		...
		search() {
            this.isBusy = true
            clearTimeout(this.stampFilterTimeout)
            this.stampFilterTimeout = setTimeout(() => {
                this.stampCriteria = this.searchTerm
                this.isBusy = false
            }, 300)
        },
		...
		getStamps() {
			this.$store.dispatch("stamps/getStamps", this.user.sponsor.id)
			.then(res => {
				this.rows = this.stamps.length
				this.isBusy = false
			})
			.catch(err => {
		...
		exportStamps() {
			...
			this.$store.dispatch("stamps/export", this.user.sponsor.id)
			.then(res => {
				const url = URL.createObjectURL(new Blob([res.data]))
                const link = document.createElement('a')
                link.href = url
                link.setAttribute('download', "stamps.xlsx")
    ...
    async fetch() {
		this.setNavbarTitle('Booth Stamps')
